...
from src.files.configs.tools.postgre import RDSPostgreSQLManager
from src.files.configs.rules.regras import rules_dict
...
class PDFTableExtractor:
    ...
    def start(self):
        ...
        main = self.sanitize_column_name(main)
        if self.configs["small_sanitize"]:
            small = self.sanitize_column_name(small)
    ...
    def get_table_data(self, t_area, t_columns, fix):
        ...
        table_content = [self.fix_header(pages.df) if fix else pages.df for pages in tables]
